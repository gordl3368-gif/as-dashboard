...
# ── 설문 폼 ───────────────────────────────────────────────────────────────────
STARS = ["★ 1점", "★★ 2점", "★★★ 3점", "★★★★ 4점", "★★★★★ 5점"]
SCORE = {s: i+1 for i, s in enumerate(STARS)}

with st.form("survey"):
    st.markdown("**1. 전체 만족도**")
    q1 = st.radio("전체만족도", STARS, index=4, horizontal=True, label_visibility="collapsed")

    st.markdown("**2. 접수 편의성** — 접수 과정이 불편하진 않으셨나요?")
    q2 = st.radio("접수편의성", STARS, index=4, horizontal=True, label_visibility="collapsed")

    st.markdown("**3. 담당자 응대** — 담당자가 친절하고 전문적이었나요?")
    q3 = st.radio("담당자응대", STARS, index=4, horizontal=True, label_visibility="collapsed")
    ...
    q5 = st.radio("안내및소통", STARS, index=4, horizontal=True, label_visibility="collapsed")
